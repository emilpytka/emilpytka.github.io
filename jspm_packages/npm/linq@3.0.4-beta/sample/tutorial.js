...
Enumerable.range(1, 3).select("$*2").log();
...
}).where("this.from($).Distinct('$.value').Count() == 5").where("$.baker != 5").where("$.cooper != 1").where("$.fletcher != 1 && $.fletcher != 5").where("$.miller > $.cooper").where("Math.abs($.smith - $.fletcher) != 1").where("Math.abs($.fletcher - $.cooper) != 1");
